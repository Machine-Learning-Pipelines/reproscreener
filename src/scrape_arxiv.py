...
def filter_category(df_full, cat1, cat2):
    ...
    df_filtered = df_full[df_full['categories'].str.contains(cat1) & df_full['categories'].str.contains(cat2)]
...
def scrape_arxiv(path_corpus, max_articles, initialize=True, date_from='2022-10-24',date_until='2022-10-25'):
    if initialize:
        ...
        df_full[df_full['categories'].str.contains('cs.lg') & df_full['categories'].str.contains('stat.ml')]
        df_filt[:max_articles].to_csv(path_corpus + 'scrape_df_' + str(max_articles)+ '.csv', index_label="index")
